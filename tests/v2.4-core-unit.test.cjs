...
const exercises=load('js/exercises/index.js');const api=exercises.window.DutchTrainer.exercises;
const word={id:'unit-1',dutch:'tekeergaan',english:'to go wild',examples:[{nl:'Hij ging tekeergaan tijdens de vergadering.'}]};
const distractor={id:'unit-2',dutch:'woeden',english:'to rage',examples:[{nl:'Hij bleef woeden.'}]};
...
check('meaning exercise asks for Dutch word',()=>{const e=api.create(word,'meaning',[word,distractor]);equal(e.correctAnswer,'tekeergaan');ok(Array.isArray(e.options)&&e.options.length>=2,'meaning options missing');});
check('recall exercise asks for English meaning',()=>{const e=api.create(word,'recall',[word]);equal(e.correctAnswer,'to go wild');});
check('fill exercise masks the exact Dutch term in a matching sentence',()=>{const e=api.create(word,'fill',[word]);ok(e.context.includes('_____'),'fill sentence gap missing');ok(!e.context.includes('tekeergaan'),'fill target remains visible');equal(e.correctAnswer,'tekeergaan');});
check('choose exercise masks the exact Dutch term and offers choices',()=>{const e=api.create(word,'choose',[word,distractor]);ok(e.context.includes('_____'),'choose sentence gap missing');ok(!e.context.includes('tekeergaan'),'choose target remains visible');ok(Array.isArray(e.options)&&e.options.length>=2,'choose options missing');equal(e.correctAnswer,'tekeergaan');});
