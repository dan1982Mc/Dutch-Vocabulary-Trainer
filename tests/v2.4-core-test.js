...
/* Dutch Vocabulary Trainer V2.4 — browser core verification. */
(function () {
    const root = window.DutchTrainer;
    const checks = [];
    ...
    check('DutchTrainer namespace exists', () => {
        if (!root) throw new Error('DutchTrainer missing');
    });

    check('DB API exists', () => {
        if (typeof root?.db?.saveWord !== 'function' || typeof root?.db?.getWords !== 'function') throw new Error('DB word API missing');
    });

    check('Vocabulary API exists', () => {
        if (typeof root?.vocabulary?.getSelected !== 'function') throw new Error('Vocabulary API missing');
    });

    check('Exercise registry contains all current types', () => {
        const expected = ['meaning', 'recall', 'fill', 'choose', 'production'];
        for (const type of expected) if (typeof root?.exercises?.get !== 'function' || !root.exercises.get(type)) throw new Error(`Missing ${type}`);
    });

    check('Practice API exists', () => {
        for (const name of ['start', 'answer', 'next', 'finish', 'reset', 'getState']) {
            if (typeof root?.practice?.[name] !== 'function') throw new Error(`Missing practice.${name}`);
        }
    });

    check('Mastery API uses DB', () => {
        if (typeof root?.mastery?.recordAnswer !== 'function') throw new Error('Mastery API missing');
        if (!root.mastery?.policy) throw new Error('Mastery policy missing');
    });

    check('History API exists', () => {
        for (const name of ['getSessions', 'saveSession', 'getRecent', 'getStats']) {
            if (typeof root?.history?.[name] !== 'function') throw new Error(`Missing history.${name}`);
        }
    });

    check('Scheduler API exists', () => {
        for (const name of ['isNew', 'isDue', 'isWeak', 'isLearned', 'schedule', 'classify']) {
            if (typeof root?.scheduler?.[name] !== 'function') throw new Error(`Missing scheduler.${name}`);
        }
    });
