...
check('import exposes validation',()=>{has('js/import.js',/window\.DutchTrainerImport=\{[\s\S]*validate\}/);});
check('import enforces word limit',()=>{has('js/import.js',/maxWords:10000/);});
check('import normalizes vocabulary to schema 3 through DB',()=>{has('js/import.js',/normalize\(w,incomingPack\.packId,i\)/);has('js/import.js',/await saveWords\(merged\)/);has('js/db.js',/schemaVersion=3/);});
check('Packs UI owns refresh renderer',()=>{has('js/packs-ui.js',/DutchTrainerPacksUI=\{refresh/);});
check('Packs navigation uses refresh',()=>{const t=read('js/ui.js');if(!/name==='packs'\)window\.DutchTrainerPacksUI\?\.refresh\?\(\)/.test(t))throw Error('nav packs refresh missing');if(!/async function setActivePack\(\)[\s\S]*?DutchTrainerPacksUI\?\.refresh\?\.\(\)/.test(t))throw Error('active pack refresh missing');});
