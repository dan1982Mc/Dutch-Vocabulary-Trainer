...
async function getAllPacks(){if(typeof getAllPackRecords!=="function")throw new Error("db.js: getAllPackRecords() is unavailable.");const packs=await getAllPackRecords();return Array.isArray(packs)?packs.filter(p=>p&&p.type!==PACK_TYPES.legacy&&p.packId!==DEFAULT_PACK_ID):[]}
...
async function deletePack(packId){const id=normalizePackId(packId);if(id===DEFAULT_PACK_ID)throw new Error("The default vocabulary pack is no longer used.");if(!(await getPack(id)))return false;if(typeof deletePackRecord!=="function")throw new Error("db.js: deletePackRecord() is unavailable.");await deletePackRecord(id);return true}
