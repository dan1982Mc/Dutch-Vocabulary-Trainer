...
async function getVocabulary(options = {}) {
    if (Array.isArray(options.vocabulary)) {
        return { words: options.vocabulary, selection: DutchTrainer.vocabulary?.getSelection?.() || { source: 'all', packId: null } };
    }
...
async function start(options = {}) {
    ...
    state = {
        ...
        finishedAt: null,
        selectedVocabulary: clone(resolved.words),
...
async function answer(answer) {
    ...
    state.answerCount++;
    if (check.correct) state.correctCount++;
