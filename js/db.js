...
async function getWord(id) {
    ...
    return requestPromise(
        transaction(STORES.vocabulary).get(id)
    );
...
async function saveWord(word) {
    ...
    return requestPromise(
        transaction(STORES.vocabulary, "readwrite").put(word)
    );
...
async function getAllWords() {
    ...
    return requestPromise(
        transaction(STORES.vocabulary).getAll()
    );
...
async function deleteWord(id) {
    ...
    return requestPromise(
        transaction(STORES.vocabulary, "readwrite").delete(id)
    );
...
async function savePack(pack) {
    ...
    return requestPromise(
        transaction(STORES.packs, "readwrite").put(pack)
    );
...
async function getPack(packId) {
    ...
    return requestPromise(
        transaction(STORES.packs).get(packId)
    );
...
async function getAllPacks() {
    ...
    return requestPromise(
        transaction(STORES.packs).getAll()
    );
...
async function deletePack(packId) {
    ...
    return requestPromise(
        transaction(STORES.packs, "readwrite").delete(packId)
    );
...
async function setSetting(key, value) {
    ...
    return requestPromise(
        ...
        transaction(STORES.settings, "readwrite").put({
...
async function getSetting(key, fallback = null) {
    ...
    const result = await requestPromise(
        transaction(STORES.settings).get(key)
    );
...
async function saveSession(session) {
    ...
    return requestPromise(
        transaction(STORES.sessions, "readwrite").put(session)
    );
...
async function getSessions() {
    ...
    return requestPromise(
        transaction(STORES.sessions).getAll()
    );
...
async function calculatePackStats(packId) {
    ...
    const packWords = words.filter(w => w.packId === packId);

    const total = packWords.length;

    const learned = packWords.filter(w => w.mastery >= 90).length;

    const weak = packWords.filter(
        w => w.mastery > 0 && w.mastery < 40
    ).length;

    const due = packWords.filter(
        w => (w.nextReview || 0) <= Date.now()
    ).length;

    const isNew = packWords.filter(w => w.isNew).length;

    const averageMastery = total
        ? Math.round(
              packWords.reduce((sum, w) => sum + (w.mastery || 0), 0) / total
          )
        : 0;
...
async function ensureLegacyPack() {

    const existing = await getPack("legacy");
    ...
    await savePack({
        ...
        author: "V1.2 Migration",

        description: "Words imported before V2.0",
...
async function initializeDB() {
    ...
    console.log("Dutch Vocabulary Trainer V2.0 database ready.");
