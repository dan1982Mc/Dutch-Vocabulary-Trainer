...
/* V2 no longer creates a Legacy Pack. Purge the old V1/V1.2 record and its vocabulary once at startup. */
async function ensureLegacyPack() { try { const existing = await getPackRecord("legacy"); if (existing) await deletePackRecord("legacy"); const words = await getAllWords(); for (const word of words) if (String(word.packId||"")==="legacy") await deleteWord(word.id); } catch (_) {} return null; }
