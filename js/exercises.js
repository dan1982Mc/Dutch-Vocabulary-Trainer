...
function distractorPool(v){if(Array.isArray(v)&&v.length>=4)return v;const full=window.DutchTrainerV2VocabularyPool;return Array.isArray(full)&&full.length?v.concat(full.filter(x=>!v.some(y=>clean(y?.id)===clean(x?.id)))):v}
function otherWords(v,w){const id=clean(w?.id);return distractorPool(v).filter(x=>clean(x?.id)!==id&&(dutch(x)||english(x)))}
