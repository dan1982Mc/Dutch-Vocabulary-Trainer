...
(async function () {
    ...
    function stats(ws) {
        ...
        return {
            total: ws.length,
            learned: ws.filter(w => (Number(w.mastery) || 0) >= 90).length,
            weak: ws.filter(w => (Number(w.mastery) || 0) < 40).length,
            due: ws.filter(w => w.isDue || (w.dueAt && new Date(w.dueAt) <= new Date())).length,
            average: ws.length ? Math.round(mastery.reduce((a,b) => a+b, 0) / ws.length) : 0
        };
    }
    ...
    async function renderHistory() {
        ...
        const rows = await app.history.getSessions(); area.innerHTML = '';
        if (!rows.length) { area.textContent = 'No practice history yet.'; return; }
        const table = document.createElement('table');
        table.innerHTML = '<thead><tr><th>Date</th><th>Exercise</th><th>Score</th></tr></thead>';
        const body = document.createElement('tbody');
        rows.slice().sort((a,b) => new Date(b.finishedAt || b.startedAt) - new Date(a.finishedAt || a.startedAt)).slice(0, 200).forEach(r => {
            const tr = document.createElement('tr');
            [new Date(r.finishedAt || r.startedAt).toLocaleString(), r.exerciseType || '', `${r.correctCount ?? 0} / ${r.answerCount ?? r.questionCount ?? 0}`].forEach(v => { const td = document.createElement('td'); td.textContent = v; tr.append(td); });
            body.append(tr);
        });
    ...
    function renderQuestion(q) {
        ...
        if (Array.isArray(e.options) && e.options.length) e.options.forEach(option => { const label=document.createElement('label'), input=document.createElement('input'); input.type='radio'; input.name='v24answer'; input.value=String(option); label.append(input, document.createTextNode(' '+option)); area.append(label); });
        else { const input=document.createElement('input'); input.id='answerInput'; input.type='text'; input.autocomplete='off'; area.append(input); input.focus(); }
        if ($('feedbackArea')) $('feedbackArea').innerHTML=''; $('checkBtn')?.classList.remove('hidden'); $('nextBtn')?.classList.add('hidden');
        const state=app.practice.state?.() || {}; if ($('sessionCounter')) $('sessionCounter').textContent=`${(state.currentIndex||0)+1}/${state.questionCount||1}`;
    }
    ...
    async function check() {
        ...
        const result = await app.practice.answer(value);
        const f = result.feedback || {};
        if ($('feedbackArea')) $('feedbackArea').textContent = f.correct ? '✓ Correct!' : `✗ Correct answer: ${f.expected || result.question?.exercise?.correctAnswer || ''}`;
    ...
    async function next() {
        ...
        if (result.completed) { nav('complete'); if ($('sessionScore')) $('sessionScore').textContent = `${result.state.correctCount || 0} / ${result.state.answerCount || 0} correct`; return; }
        renderQuestion(result.question);
    }

    function bind() {
        $('dashboardBtn')?.addEventListener('click', () => nav('dashboard'));
        $('packsBtn')?.addEventListener('click', () => nav('packs'));
        $('historyBtn')?.addEventListener('click', () => nav('history'));
        ...
        $('quickPracticeBtn')?.addEventListener('click', startPractice);
        $('practiceSetupBtn')?.addEventListener('click', () => $('practiceModal')?.classList.remove('hidden'));
        $('closePracticeModal')?.addEventListener('click', () => $('practiceModal')?.classList.add('hidden'));
        $('startPracticeBtn')?.addEventListener('click', () => { $('practiceModal')?.classList.add('hidden'); startPractice(); });
        $('checkBtn')?.addEventListener('click', check);
        $('nextBtn')?.addEventListener('click', next);
        $('exitPracticeBtn')?.addEventListener('click', () => nav('home'));
        $('backDashboardBtn')?.addEventListener('click', () => nav('dashboard'));
